...
class ProductRegistrationActivity
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // supportActionBar?.title 프로퍼티는 상단 액션바에 표시될 제목 텍스트
        supportActionBar?.title = "상품 등록"
    }
    // ProductRegistrationActivity에는 UI에서 따로 툴바를 만들지 않을 예정이기 때문에
    // onOptionsItemSelected() 콜백을 이용해 기본 액션바의 좌측 버튼이 눌렀을 때 액션을 정의함
    // 여기에서 android.R.id.home 은 기본 액션바를 사용할 때 액션바 좌측의 버튼 아이디를 지칭한다.
    // 이 버튼이 눌렀을 때 시스템의 백버튼을 눌렀을 때와 같은 효과를 내기 위해
    // onBackPressed()함수를 호출함
